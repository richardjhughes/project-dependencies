...
def getFullInstallDir(path, buildForiOS, buildForiOSSimulator):
    ...
    installDir = os.path.join(path, "sdl_ttf", platformName)
...
def isSDLAlreadyInstalled(path, buildForiOS, buildForiOSSimulator):
    ...
    if platform.system() == "Windows":
        filesExist = (os.path.exists(os.path.join(installDir, "SDL2_ttf.dll")) and
                      os.path.exists(os.path.join(installDir, "SDL2_ttf.lib")) and
                      os.path.exists(os.path.join(installDir, "include")))
    else:
        filesExist = (os.path.exists(os.path.join(installDir, "libSDL2_ttf.a")) and
                      os.path.exists(os.path.join(installDir, "include")))
...
def getOutputZipPath(binaryOutPath, buildForiOS, buildForiOSSimulator):
    ...
    zipPath = os.path.join(binaryOutPath, f"{sdlTTFVersion}_{platformName}.zip")
...
if isSDLAlreadyInstalled(args.path, args.ios, args.ios_simulator):
    print("SDL ttf already installed.")
else:
